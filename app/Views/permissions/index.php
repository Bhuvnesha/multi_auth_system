...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-key"></i> Permission Management</h2>
    ...
    <div class="card-body">
        <?php if (empty($permissions)): ?>
            <div class="text-center py-4">
                ...
                <p class="mt-3 text-muted">No permissions found.</p>
            </div>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($permissions as $permission): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="badge bg-secondary"><?= esc($permission->resource) ?></span>
                                </td>
                                <td>
                                    <span class="badge bg-<?= $permission->action == 'manage' ? 'danger' : 'primary' ?>">
                                        <?= ucfirst($permission->action) ?>
                                    </span>
                                </td>
                                <td>
                                    <span class="badge bg-success"><?= $permission->role_count ?? 0 ?> roles</span>
                                </td>
                                <td>
                                    <div class="btn-group btn-group-sm action-buttons">
                                        ...
                                        <?php if (!$permission->is_system): ?>
                                            <button class="btn btn-outline-danger delete-confirm"
                                                    ...
                                                    data-delete-url="/permissions/delete/<?= $permission->id ?>">
